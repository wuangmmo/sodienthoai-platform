...
func main() {
	...
	searchURL := os.Getenv("OPENSEARCH_URL")
...
data_quality_score::float8,last_seen_at FROM phone_numbers ORDER BY id`)
	...
	for rows.Next() {
		...
		body,err:=search.MarshalPhoneDocument(d); if err!=nil { log.Fatal(err) }
		if err:=client.IndexPhone(ctx,d.ID,body); err!=nil { log.Fatal(err) }
		count++
	}
	if err:=rows.Err(); err!=nil { log.Fatal(err) }
	log.Printf("reindexed %d phone numbers",count)
}
