...
func (s Service) ModerateReport(ctx context.Context,id,status string) error {
 ...
 if err:=s.Repository.RefreshDerivedStatus(ctx,phoneID);err!=nil{return err};s.invalidatePhoneCache(ctx,phoneID);_ = s.NotifyFollowers(ctx,phoneID,"reputation_changed",map[string]string{"source":"report_moderation","status":status});return nil
}
func (s Service) ModerateClaim(ctx context.Context,id,status string) error {
 ...
 if err:=s.Repository.RefreshDerivedStatus(ctx,phoneID);err!=nil{return err};s.invalidatePhoneCache(ctx,phoneID);_ = s.NotifyFollowers(ctx,phoneID,"identity_verification_changed",map[string]string{"source":"claim_moderation","status":status});return nil
}
