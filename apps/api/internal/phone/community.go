...
func(s Service) HelpfulComment(ctx context.Context,subject,commentID string)error{
 ...
 res,err:=s.Repository.DB.ExecContext(ctx,`INSERT INTO comment_helpful_votes(comment_id,user_id) SELECT id,$2 FROM phone_comments WHERE id=$1 AND status='approved' ON CONFLICT DO NOTHING`,commentID,uid);if err!=nil{return err}
 if n,_:=res.RowsAffected();n==0{return ErrInvalidComment}
 _,err=s.Repository.DB.ExecContext(ctx,`UPDATE phone_comments SET helpful_count=helpful_count+1 WHERE id=$1 AND status='approved'`,commentID);return err
}
...
func(s Service) ReportComment(ctx context.Context,subject,commentID,reason string)error{
 ...
 uid,err:=s.EnsureUser(ctx,subject);if err!=nil{return err}
 res,err:=s.Repository.DB.ExecContext(ctx,`INSERT INTO comment_reports(comment_id,user_id,reason) SELECT id,$2,$3 FROM phone_comments WHERE id=$1 AND status='approved' ON CONFLICT(comment_id,user_id) DO NOTHING`,commentID,uid,reason);if err!=nil{return err}
 if n,_:=res.RowsAffected();n==0{return ErrInvalidComment};return nil
}
