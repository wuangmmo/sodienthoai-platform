...
func(h BusinessHandler) Mine(w http.ResponseWriter,r *http.Request){
 ...
 items:=[]map[string]any{};for rows.Next(){var id,slug,name,verification,role,ownership string;var updated time.Time;if err:=rows.Scan(&id,&slug,&name,&verification,&role,&ownership,&updated);err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};items=append(items,map[string]any{"id":id,"slug":slug,"display_name":name,"verification_status":verification,"role":role,"ownership_status":ownership,"updated_at":updated})};if err:=rows.Err();err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return}
...
func(h BusinessHandler) MyVerifications(w http.ResponseWriter,r *http.Request){
 ...
 items:=[]map[string]any{};for rows.Next(){var id,bid,name,method,status string;var created time.Time;var reviewed *time.Time;if err:=rows.Scan(&id,&bid,&name,&method,&status,&created,&reviewed);err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};items=append(items,map[string]any{"id":id,"business_id":bid,"display_name":name,"method":method,"status":status,"created_at":created,"reviewed_at":reviewed})};if err:=rows.Err();err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return}
 writeJSON(w,200,map[string]any{"data":items})
}
