...
func(a AdminHandler) Appeals(w http.ResponseWriter,r *http.Request){
 ...
 items:=[]map[string]any{};for rows.Next(){var id,e164,reason,statement,status string;var created time.Time;if err:=rows.Scan(&id,&e164,&reason,&statement,&status,&created);err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};items=append(items,map[string]any{"id":id,"e164":e164,"reason":reason,"statement":statement,"status":status,"created_at":created})};if err:=rows.Err();err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};writeJSON(w,200,map[string]any{"data":items})
}
func(a AdminHandler) BusinessVerifications(w http.ResponseWriter,r *http.Request){
 ...
 items:=[]map[string]any{};for rows.Next(){var id,bid,name,method,status string;var statement *string;var created time.Time;if err:=rows.Scan(&id,&bid,&name,&method,&status,&statement,&created);err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};items=append(items,map[string]any{"id":id,"business_id":bid,"business_name":name,"method":method,"status":status,"statement":statement,"created_at":created})};if err:=rows.Err();err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};writeJSON(w,200,map[string]any{"data":items})
}
func(a AdminHandler) BusinessReviews(w http.ResponseWriter,r *http.Request){
 ...
 items:=[]map[string]any{};for rows.Next(){var id,bid,name,status string;var rating int;var body *string;var created time.Time;if err:=rows.Scan(&id,&bid,&name,&rating,&body,&status,&created);err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};items=append(items,map[string]any{"id":id,"business_id":bid,"business_name":name,"rating":rating,"body":body,"status":status,"created_at":created})};if err:=rows.Err();err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};writeJSON(w,200,map[string]any{"data":items})
}
