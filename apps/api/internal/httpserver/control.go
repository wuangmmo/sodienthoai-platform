...
func (h ControlHandler) Organizations(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};ctx,cancel:=context.WithTimeout(r.Context(),3*time.Second);defer cancel();rows,err:=h.DB.QueryContext(ctx,`SELECT DISTINCT o.id::text,o.name,o.slug FROM organizations o JOIN control_admin_assignments ca ON ca.organization_id=o.id WHERE ca.admin_user_id=$1::uuid AND ca.site_id IS NULL ORDER BY o.name`,a.Admin.ID);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer rows.Close();type item struct{ID,Name,Slug string};items:=[]item{};for rows.Next(){var x item;if rows.Scan(&x.ID,&x.Name,&x.Slug)==nil{items=append(items,x)}};writeJSON(w,200,map[string]any{"organizations":items})}
...
type controlSiteInput struct { OrganizationID string `json:"organizationId"`; Name string `json:"name"`; Slug string `json:"slug"`; Domain string `json:"domain"`; SiteType string `json:"siteType"`; ManagementMode string `json:"managementMode"`; Status string `json:"status"` }
type controlRoleInput struct { OrganizationID string `json:"organizationId"`; Name string `json:"name"`; Slug string `json:"slug"`; Description string `json:"description"`; Permissions []string `json:"permissions"` }
...
func (h ControlHandler) CreateSite(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["sites.manage"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};var in controlSiteInput;if json.NewDecoder(http.MaxBytesReader(w,r.Body,32<<10)).Decode(&in)!=nil{writeJSON(w,400,map[string]string{"error":"invalid_request"});return};in.Name=strings.TrimSpace(in.Name);in.Slug=strings.ToLower(strings.TrimSpace(in.Slug));in.Domain=strings.ToLower(strings.TrimSpace(in.Domain));if !validSiteInput(in){writeJSON(w,400,map[string]string{"error":"invalid_site"});return};ctx,cancel:=context.WithTimeout(r.Context(),4*time.Second);defer cancel();tx,err:=h.DB.BeginTx(ctx,nil);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer tx.Rollback();var id string;var allowedOrg bool;err=tx.QueryRowContext(ctx,`SELECT EXISTS(SELECT 1 FROM control_admin_assignments WHERE admin_user_id=$1::uuid AND organization_id=$2::uuid AND site_id IS NULL)`,a.Admin.ID,in.OrganizationID).Scan(&allowedOrg);if err==nil&&!allowedOrg{writeJSON(w,403,map[string]string{"error":"organization_scope_forbidden"});return};if err==nil{err=tx.QueryRowContext(ctx,`INSERT INTO sites(organization_id,slug,name,domain,site_type,management_mode,status) VALUES($1::uuid,$2,$3,$4,$5,$6,$7) RETURNING id::text`,in.OrganizationID,in.Slug,in.Name,in.Domain,in.SiteType,in.ManagementMode,in.Status).Scan(&id)};if err==nil{err=auditTx(ctx,tx,a,r,"site.create","site",id,nil,in)};if err!=nil||tx.Commit()!=nil{writeJSON(w,400,map[string]string{"error":"site_create_failed"});return};writeJSON(w,201,map[string]any{"id":id})}

func (h ControlHandler) UpdateSite(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["sites.manage"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};id:=r.PathValue("id");var in controlSiteInput;if json.NewDecoder(http.MaxBytesReader(w,r.Body,32<<10)).Decode(&in)!=nil{writeJSON(w,400,map[string]string{"error":"invalid_request"});return};in.Name=strings.TrimSpace(in.Name);in.Slug=strings.ToLower(strings.TrimSpace(in.Slug));in.Domain=strings.ToLower(strings.TrimSpace(in.Domain));if !validSiteInput(in){writeJSON(w,400,map[string]string{"error":"invalid_site"});return};ctx,cancel:=context.WithTimeout(r.Context(),4*time.Second);defer cancel();tx,err:=h.DB.BeginTx(ctx,nil);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer tx.Rollback();var before map[string]any=map[string]any{};var oldName,oldDomain,oldType,oldMode,oldStatus string;err=tx.QueryRowContext(ctx,`SELECT name,domain,site_type,management_mode,status FROM sites WHERE id=$1::uuid AND id::text=ANY($2::text[])`,id,pqTextArray(a.SiteIDs)).Scan(&oldName,&oldDomain,&oldType,&oldMode,&oldStatus);before=map[string]any{"name":oldName,"domain":oldDomain,"siteType":oldType,"managementMode":oldMode,"status":oldStatus};if err==nil{_,err=tx.ExecContext(ctx,`UPDATE sites SET name=$1,domain=LOWER($2),site_type=$3,management_mode=$4,status=$5,updated_at=NOW() WHERE id=$6::uuid`,strings.TrimSpace(in.Name),strings.TrimSpace(in.Domain),in.SiteType,in.ManagementMode,in.Status,id)};if err==nil{err=auditTx(ctx,tx,a,r,"site.update","site",id,before,in)};if err==sql.ErrNoRows{writeJSON(w,404,map[string]string{"error":"site_not_found"});return};if err!=nil||tx.Commit()!=nil{writeJSON(w,400,map[string]string{"error":"site_update_failed"});return};writeJSON(w,200,map[string]bool{"ok":true})}

func requestedPermissionsAllowed(a controlAccess, requested []string) bool { for _,p:=range requested { if !a.Permissions[p] { return false } }; return true }

func (h ControlHandler) CreateRole(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["roles.manage"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};var in controlRoleInput;if json.NewDecoder(http.MaxBytesReader(w,r.Body,64<<10)).Decode(&in)!=nil||strings.TrimSpace(in.Name)==""||strings.TrimSpace(in.Slug)==""{writeJSON(w,400,map[string]string{"error":"invalid_request"});return};if !requestedPermissionsAllowed(a,in.Permissions){writeJSON(w,403,map[string]string{"error":"permission_escalation"});return};ctx,cancel:=context.WithTimeout(r.Context(),5*time.Second);defer cancel();tx,err:=h.DB.BeginTx(ctx,nil);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer tx.Rollback();var id string;var allowedOrg bool;err=tx.QueryRowContext(ctx,`SELECT EXISTS(SELECT 1 FROM control_admin_assignments WHERE admin_user_id=$1::uuid AND organization_id=$2::uuid AND site_id IS NULL)`,a.Admin.ID,in.OrganizationID).Scan(&allowedOrg);if err==nil&&!allowedOrg{writeJSON(w,403,map[string]string{"error":"organization_scope_forbidden"});return};if err==nil{err=tx.QueryRowContext(ctx,`INSERT INTO control_roles(organization_id,slug,name,description) VALUES($1::uuid,LOWER($2),$3,$4) RETURNING id::text`,in.OrganizationID,strings.TrimSpace(in.Slug),strings.TrimSpace(in.Name),strings.TrimSpace(in.Description)).Scan(&id)};for _,p:=range in.Permissions{if err==nil{_,err=tx.ExecContext(ctx,`INSERT INTO control_role_permissions(role_id,permission_key) VALUES($1::uuid,$2)`,id,p)}};if err==nil{err=auditTx(ctx,tx,a,r,"role.create","role",id,nil,in)};if err!=nil||tx.Commit()!=nil{writeJSON(w,400,map[string]string{"error":"role_create_failed"});return};writeJSON(w,201,map[string]any{"id":id})}

func (h ControlHandler) UpdateRole(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["roles.manage"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};id:=r.PathValue("id");var in controlRoleInput;if json.NewDecoder(http.MaxBytesReader(w,r.Body,64<<10)).Decode(&in)!=nil{writeJSON(w,400,map[string]string{"error":"invalid_request"});return};if !requestedPermissionsAllowed(a,in.Permissions){writeJSON(w,403,map[string]string{"error":"permission_escalation"});return};ctx,cancel:=context.WithTimeout(r.Context(),5*time.Second);defer cancel();tx,err:=h.DB.BeginTx(ctx,nil);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer tx.Rollback();var system bool;var oldName,oldDesc string;err=tx.QueryRowContext(ctx,`SELECT name,description,is_system FROM control_roles WHERE id=$1::uuid AND organization_id IN(SELECT organization_id FROM control_admin_assignments WHERE admin_user_id=$2::uuid AND site_id IS NULL)`,id,a.Admin.ID).Scan(&oldName,&oldDesc,&system);if system { writeJSON(w,http.StatusConflict,map[string]string{"error":"system_role_immutable"}); return };if err==nil{_,err=tx.ExecContext(ctx,`UPDATE control_roles SET name=$1,description=$2,updated_at=NOW() WHERE id=$3::uuid`,strings.TrimSpace(in.Name),strings.TrimSpace(in.Description),id)};if err==nil{_,err=tx.ExecContext(ctx,`DELETE FROM control_role_permissions WHERE role_id=$1::uuid`,id)};for _,p:=range in.Permissions{if err==nil{_,err=tx.ExecContext(ctx,`INSERT INTO control_role_permissions(role_id,permission_key) VALUES($1::uuid,$2)`,id,p)}};if err==nil{err=auditTx(ctx,tx,a,r,"role.update","role",id,map[string]any{"name":oldName,"description":oldDesc},in)};if err!=nil||tx.Commit()!=nil{writeJSON(w,400,map[string]string{"error":"role_update_failed"});return};writeJSON(w,200,map[string]bool{"ok":true})}

func (h ControlHandler) DeleteRole(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["roles.manage"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};id:=r.PathValue("id");ctx,cancel:=context.WithTimeout(r.Context(),4*time.Second);defer cancel();tx,err:=h.DB.BeginTx(ctx,nil);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer tx.Rollback();var name string;var system bool;err=tx.QueryRowContext(ctx,`SELECT name,is_system FROM control_roles WHERE id=$1::uuid AND organization_id IN (SELECT organization_id FROM control_admin_assignments WHERE admin_user_id=$2::uuid AND organization_id IS NOT NULL AND site_id IS NULL)`,id,a.Admin.ID).Scan(&name,&system);if system{writeJSON(w,409,map[string]string{"error":"system_role"});return};if err==nil{err=auditTx(ctx,tx,a,r,"role.delete","role",id,map[string]any{"name":name},nil)};if err==nil{_,err=tx.ExecContext(ctx,`DELETE FROM control_roles WHERE id=$1::uuid`,id)};if err==sql.ErrNoRows{writeJSON(w,404,map[string]string{"error":"role_not_found"});return};if err!=nil||tx.Commit()!=nil{writeJSON(w,400,map[string]string{"error":"role_delete_failed"});return};writeJSON(w,200,map[string]bool{"ok":true})}

func (h ControlHandler) AssignScope(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["admins.manage"]&&!a.Permissions["roles.manage"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};var in controlScopeInput;if json.NewDecoder(http.MaxBytesReader(w,r.Body,32<<10)).Decode(&in)!=nil{writeJSON(w,400,map[string]string{"error":"invalid_request"});return};ctx,cancel:=context.WithTimeout(r.Context(),4*time.Second);defer cancel();tx,err:=h.DB.BeginTx(ctx,nil);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer tx.Rollback();var allowedOrg bool;err=tx.QueryRowContext(ctx,`SELECT EXISTS(SELECT 1 FROM control_admin_assignments WHERE admin_user_id=$1::uuid AND organization_id=$2::uuid AND site_id IS NULL)`,a.Admin.ID,in.OrganizationID).Scan(&allowedOrg);if err==nil&&!allowedOrg{writeJSON(w,403,map[string]string{"error":"scope_escalation"});return};if err==nil&&in.SiteID!=nil{var allowedSite bool;err=tx.QueryRowContext(ctx,`SELECT EXISTS(SELECT 1 FROM sites WHERE id=$1::uuid AND organization_id=$2::uuid AND id::text=ANY($3::text[]))`,*in.SiteID,in.OrganizationID,pqTextArray(a.SiteIDs)).Scan(&allowedSite);if err==nil&&!allowedSite{writeJSON(w,403,map[string]string{"error":"scope_escalation"});return}};if err==nil{var roleSystem bool;var roleSlug string;err=tx.QueryRowContext(ctx,`SELECT is_system,slug FROM control_roles WHERE id=$1::uuid AND organization_id=$2::uuid`,in.RoleID,in.OrganizationID).Scan(&roleSystem,&roleSlug);if err==nil&&roleSystem{var actorSuper bool;err=tx.QueryRowContext(ctx,`SELECT EXISTS(SELECT 1 FROM control_admin_assignments ca JOIN control_roles r ON r.id=ca.role_id WHERE ca.admin_user_id=$1::uuid AND ca.organization_id=$2::uuid AND ca.site_id IS NULL AND r.slug='super-admin')`,a.Admin.ID,in.OrganizationID).Scan(&actorSuper);if err==nil&&!actorSuper{writeJSON(w,403,map[string]string{"error":"system_role_assignment_forbidden"});return}};if err==nil&&roleSlug=="super-admin"{var actorSuper bool;err=tx.QueryRowContext(ctx,`SELECT EXISTS(SELECT 1 FROM control_admin_assignments ca JOIN control_roles r ON r.id=ca.role_id WHERE ca.admin_user_id=$1::uuid AND ca.organization_id=$2::uuid AND ca.site_id IS NULL AND r.slug='super-admin')`,a.Admin.ID,in.OrganizationID).Scan(&actorSuper);if err==nil&&!actorSuper{writeJSON(w,403,map[string]string{"error":"super_admin_assignment_forbidden"});return}}};if in.AdminUserID==a.Admin.ID { var roleSlug string; if err==nil{err=tx.QueryRowContext(ctx,`SELECT slug FROM control_roles WHERE id=$1::uuid`,in.RoleID).Scan(&roleSlug)};if err==nil&&roleSlug!="super-admin"{writeJSON(w,409,map[string]string{"error":"self_privilege_change_forbidden"});return} };var id string;if err==nil{err=tx.QueryRowContext(ctx,`INSERT INTO control_admin_assignments(admin_user_id,role_id,organization_id,site_id) VALUES($1::uuid,$2::uuid,$3::uuid,$4::uuid) RETURNING id::text`,in.AdminUserID,in.RoleID,in.OrganizationID,in.SiteID).Scan(&id)};if err==nil{err=auditTx(ctx,tx,a,r,"scope.assign","admin_assignment",id,nil,in)};if err!=nil||tx.Commit()!=nil{writeJSON(w,400,map[string]string{"error":"scope_assign_failed"});return};writeJSON(w,201,map[string]any{"id":id})}
...
func (h ControlHandler) Admins(w http.ResponseWriter,r *http.Request){
	...
	ctx,cancel:=context.WithTimeout(r.Context(),3*time.Second);defer cancel()
	rows,err:=h.DB.QueryContext(ctx,`SELECT DISTINCT au.id::text,au.name,au.email,au.status FROM admin_users au JOIN control_admin_assignments ca ON ca.admin_user_id=au.id WHERE ca.organization_id IN (SELECT organization_id FROM control_admin_assignments WHERE admin_user_id=$1::uuid AND organization_id IS NOT NULL AND site_id IS NULL) ORDER BY au.email`,a.Admin.ID)
...
func (h ControlHandler) Audit(w http.ResponseWriter,r *http.Request){a,ok:=h.access(r);if !ok{writeJSON(w,401,map[string]string{"error":"unauthorized"});return};if !a.Permissions["audit.view"]{writeJSON(w,403,map[string]string{"error":"forbidden"});return};ctx,cancel:=context.WithTimeout(r.Context(),3*time.Second);defer cancel();rows,err:=h.DB.QueryContext(ctx,`SELECT l.id,l.action,l.resource_type,COALESCE(l.resource_id,''),COALESCE(a.email,''),l.created_at FROM control_audit_logs l LEFT JOIN admin_users a ON a.id=l.admin_user_id WHERE l.admin_user_id=$1::uuid OR EXISTS(SELECT 1 FROM control_admin_assignments ca WHERE ca.admin_user_id=$1::uuid AND ca.organization_id=l.organization_id AND (ca.site_id IS NULL OR (l.site_id IS NOT NULL AND ca.site_id=l.site_id))) ORDER BY l.created_at DESC LIMIT 100`,a.Admin.ID);if err!=nil{writeJSON(w,500,map[string]string{"error":"internal_error"});return};defer rows.Close();type item struct{ID int64;Action,ResourceType,ResourceID,Actor string;CreatedAt time.Time};items:=[]item{};for rows.Next(){var x item;if rows.Scan(&x.ID,&x.Action,&x.ResourceType,&x.ResourceID,&x.Actor,&x.CreatedAt)==nil{items=append(items,x)}};writeJSON(w,200,map[string]any{"audit":items})}
