...
export default async function sitemap({id}:{id:number}):Promise<MetadataRoute.Sitemap>{
  ...
  try{const res=await fetch(API+`/v1/seo/sitemap?limit=50000&page=${id}`,{next:{revalidate:3600}});if(!res.ok)return urls;const body:{data:{e164:string;updated_at?:string}[]}=await res.json();for(const item of body.data)urls.push({url:SITE+"/phone/"+encodeURIComponent(item.e164),lastModified:item.updated_at?new Date(item.updated_at):undefined,changeFrequency:"weekly",priority:.6})}catch{}
  return urls;
}
