...
export async function generateMetadata({params,searchParams}:{params:Promise<{number:string}>;searchParams:Promise<{country?:string}>}):Promise<Metadata>{
 ...
 return{title:primary?`${primary.display_name} - ${p.e164}`:`Số điện thoại ${p.e164}`,description:primary?.description||`Tra cứu thông tin, trạng thái xác minh và báo cáo của số điện thoại ${p.e164}.`,alternates:{canonical:canonical(p.e164)},robots:{index:indexable,follow:true},openGraph:{title:primary?`${primary.display_name} - ${p.e164}`:`Số điện thoại ${p.e164}`,description:primary?.description||`Tra cứu thông tin số điện thoại ${p.e164}.`,type:"website",url:canonical(p.e164)}};
}
export default async function PhonePage({params,searchParams}:{params:Promise<{number:string}>;searchParams:Promise<{country?:string}>}){
 ...
 const d=result.data!,p=d.number;if(decodeURIComponent(number)!==p.e164)redirect(canonical(p.e164));const primary=d.identities.find(i=>i.is_primary);const structured={"@context":"https://schema.org","@type":primary?.kind==="business"?"Organization":"WebPage",name:primary?.display_name||`Số điện thoại ${p.e164}`,url:`https://sodienthoai.com${canonical(p.e164)}`,description:primary?.description||`Thông tin tra cứu số điện thoại ${p.e164}`};
 return <main className="wrap result"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structured).replace(/</g,"\\u003c")}}/><h1>{primary?.display_name||p.e164}</h1>{primary?<p className="notice">{p.e164} · Độ tin cậy {primary.confidence_score}/100</p>:<p className="notice">Số điện thoại này chưa được định danh. Bạn có thể gửi thông tin xác minh hoặc báo cáo nếu biết về số này.</p>}
