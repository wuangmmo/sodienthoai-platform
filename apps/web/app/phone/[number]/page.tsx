...
async function lookup(number:string):Promise<{data?:PhoneData;notFound?:boolean}>{
  const base=process.env.API_INTERNAL_BASE_URL||process.env.NEXT_PUBLIC_API_BASE_URL||"http://localhost:8080";
  const res=await fetch(base+"/v1/phone/"+encodeURIComponent(number),{cache:"no-store"});
...
export default async function PhonePage({params}:{params:Promise<{number:string}>}){
  const {number}=await params; let result;
  try{result=await lookup(decodeURIComponent(number));}catch{return <main className="wrap result"><h1>Không thể tra cứu lúc này</h1><p className="notice">Dịch vụ đang tạm thời không khả dụng. Vui lòng thử lại sau.</p><Link className="back" href="/">← Tra cứu số khác</Link></main>}
