...
export default async function PhonePage({params,searchParams}:{params:Promise<{number:string}>;searchParams:Promise<{country?:string}>}){
 ...
 return <main className="wrap result"><h1>{primary?.display_name||p.e164}</h1>{primary?<p className="notice">{p.e164} · Độ tin cậy {primary.confidence_score}/100</p>:<p className="notice">Số điện thoại này chưa được định danh. Bạn có thể gửi thông tin xác minh hoặc báo cáo nếu biết về số này.</p>}
